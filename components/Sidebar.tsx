  ...
  const navSections = [
    {
      ...
      items: [
        ...
        { id: 'email-hub', label: 'Centro de Correos', icon: <Mail size={20} /> },
    ...
    },
    {
      ...
      items: [
        { id: 'swarm-orchestrator', label: 'Automatización', icon: <Network size={20} /> },
        { id: 'negotiation-hub', label: 'Negociación', icon: <Briefcase size={20} /> },
      ]
    },
    {
      title: 'GROWTH',
      items: [
        ...
        { id: 'neuro-finance', label: 'Finanzas', icon: <Wallet size={20} /> },
        { id: 'funnels', label: 'Embudos', icon: <Filter size={20} /> },
    ...
    },
    {
      ...
      items: [
        { id: 'ontology-core', label: 'Ontologías', icon: <Share2 size={20} /> },
        { id: 'system-health', label: 'Auditoría', icon: <HeartPulse size={20} /> },
    ...
    >
      ...
      <div className={`flex flex-col gap-1 w-full py-4 border-t border-white/[0.04] bg-[#0F0F12]/50 backdrop-blur transition-all ${isCollapsed ? 'items-center' : 'px-3 items-start'}`}>
        ...
        <div className={`flex ${isCollapsed ? 'flex-col gap-2' : 'gap-1 px-2 mb-2 w-full justify-start'}`}>
          <button className="p-2 text-obsidian-text-muted hover:text-white hover:bg-white/10 rounded transition-colors" title="Notificaciones">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9" /><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0" /></svg>
          </button>
          <button className="p-2 text-obsidian-text-muted hover:text-white hover:bg-white/10 rounded transition-colors" title="Notas Rápidas">
          ...
          <button className="p-2 text-obsidian-text-muted hover:text-white hover:bg-white/10 rounded transition-colors" title="Búsqueda Global">
        ...
        <button className={`flex items-center gap-3 text-obsidian-text-muted hover:text-white transition-colors p-2.5 rounded-lg hover:bg-white/[0.04] w-full ${isCollapsed ? 'justify-center' : ''}`} title="Configuración">
          <Settings size={18} strokeWidth={1.5} />
          {!isCollapsed && <span className="text-xs">Configuración</span>}
        </button>
        <button onClick={onLogout} className={`flex items-center gap-3 text-obsidian-text-muted hover:text-red-400 transition-colors p-2.5 rounded-lg hover:bg-red-500/10 w-full ${isCollapsed ? 'justify-center' : ''}`} title="Cerrar Sesión">
